...
phase_controller_candidate_spec
  ↓
k00_route_status = K00_INTAKE_ACCEPTED | K00_ROUTE_RECOVERY_DOCUMENTED | K00_ROUTE_FAILED
  ↓
k00_to_p00_handoff_packet
```

K00 route status 是所有后续 Phase Controller / Integration Program READY 结论的前置字段：

```text
只有 k00_route_status 为 K00_INTAKE_ACCEPTED 或 K00_ROUTE_RECOVERY_DOCUMENTED，PXX / IXX / Phase Controller READY 才允许被验收为有效。
如果用户上传或粘贴系统建设资料后，未完成 raw / passport / registry / corpus index / system mapping / phase mapping / gap / task package / handoff，则判定为 K00_ROUTE_FAILED，必须先 route recovery，再进入阶段验收。
```
...
# Appendix: Methodology v3 Supplement Anchor

本蓝图已由 `methodology_v3_bootstrap_full_control_plane_supplement.md` 补充。

补充主题：Methodology v3 Supplement: Bootstrap Control Plane / Full Control Plane Split。

权威裁决：Control Plane 拆分为 Bootstrap Control Plane 与 Full Control Plane；当前下一合法阶段为 `SYSTEM_PLANES_PROFESSIONAL_GENERATION_AND_ACCEPTANCE`；P01 在系统平面专业化生成与验收通过前继续阻断；系统保持 `paper_only=true`、`real_trade_enabled=false`、`auto_order_allowed=false`。



---

# Appendix — KV Cache Enhanced Document Processing Method

- source_doc_id: `DOC-20260513-001`
- canonical_addendum: `00_methodology/kv_cache_document_processing_method.md`
- status: `KV_CACHE_METHOD_ACCEPTED_IMPLEMENTATION_REQUIRED`

新版文档处理方式必须把 KV Cache 作为 K00 的结构化语义缓存层：每份系统建设资料除了 raw input、document passport、system mapping、gap detection、task package、handoff packet 之外，还应生成 `kv_items_<doc_id>.jsonl` 与 `kv_cache_manifest_<doc_id>.json`。

KV Cache 的用途不是聊天记忆或普通摘要，而是把判断逻辑、字段需求、反证规则、量化模型、行为推断、输出模板、阶段合约、任务节点等稳定知识单元转成可复用、可追踪、可验收的 key-value 资产。

在 K00 contract/runner 未完成实现前，系统状态不得声称新版文档处理完全闭环，只能标记为：

```text
K00_READY_WITH_KV_GAP
```

下游 P00/P01-P09 消费文档资产时，应优先引用 `kv_item.key`，并在实现、证伪、升级、废弃时回写 consumed/implemented/superseded trace。
