...
from .token_data_display_panel import build_token_data_panel
...
def cmd_token_data_panel(args: argparse.Namespace) -> int:
    panel = build_token_data_panel(
        args.token_address,
        mode=args.mode,
        root=Path(args.root),
        write=not args.no_write,
    ...
    if panel.quality_status not in {"PASS", "PASS_WITH_WARNING"}:
        return 2
    if panel.forbidden_term_scan.get("status") != "PASS":
        return 3
    return 0
...
def build_parser() -> argparse.ArgumentParser:
    ...
    p = sub.add_parser("token-data-panel")
    p.add_argument("--token-address", required=True)
    p.add_argument("--mode", default="live", choices=["ad_hoc", "archive", "legacy", "live", "live_test"])
    p.add_argument("--root", default="/root/sikk-gmgn")
    p.add_argument("--no-write", action="store_true")
    p.set_defaults(func=cmd_token_data_panel)
