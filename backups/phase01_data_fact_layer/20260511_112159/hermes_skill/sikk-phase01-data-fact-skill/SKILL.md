...
### Skill vs Data vs Runtime Code

Do not confuse the Phase 01 skill with the token data directory.

```text
skill = reusable operating knowledge: boundaries, contracts, schema expectations, quality gates, handoff rules, and anti-pollution rules.
runtime code = executable collectors/normalizers under /root/sikk-gmgn/modules/source_wallet_bot/.
data fact store = per-token raw/normalized/handoff artifacts under /root/sikk-gmgn/data/source_wallet_bot/<mode>/<token_address>/.
```

Correct usage:

```text
Load skill → decide allowed source/fields/output boundary → run or read runtime code/data → write token facts under data/source_wallet_bot/<mode>/<token_address>/ → produce standard handoff artifacts.
```

Incorrect usage:

```text
Treating /root/sikk-gmgn/data/source_wallet_bot/live/ as the skill itself.
```

Reason: `live/` is a mutable fact cache and token snapshot store. It contains evidence for specific tokens and changes over time. A Hermes skill must remain a stable reusable procedure and governance layer. Runtime data may be referenced by the skill but must not be embedded into the skill as the skill body.

Recommended mode semantics:

- `live/<token_address>/`: canonical production-like current token fact snapshots and standard Phase 01 outputs.
- `live_test/<token_address>/`: experiments, acceptance tests, fixture-like runtime validation.
- `archive/<token_address>/<snapshot_id>/` or dated run folders: historical immutable snapshots when needed.
...
4. **Run the Phase 01 fact-store router before reading token data**:

   ```bash
   cd /root/sikk-gmgn
   python -m modules.source_wallet_bot.runner index-fact-store \
     --mode <live|live_test> \
   ...
   ```text
   data/source_wallet_bot/<mode>/<token_address>/manifest/phase01_fact_store_index.json
   ```

   Purpose:

   - bind this skill to the real runtime data store;
   - locate canonical raw/normalized/intelligence/handoff files;
   - distinguish primary vs fallback paths;
   - list `missing_required_assets` and `missing_optional_assets`;
   - set `quality_status`: `PASS`, `PASS_WITH_WARNING`, `PAUSE`, or `BLOCK`;
   - define downstream read policy and forbidden bypasses.

   HER must read `phase01_fact_store_index.json` first when analyzing a token. If the router returns `PAUSE` or `BLOCK`, do not fabricate token analysis; either complete the missing required Source Wallet Bot artifacts or report the blocker explicitly.
5. Read contracts and schemas before writing or changing output design:
...
6. Inventory available raw sources.
7. Normalize each fact table with source provenance.
8. Cross-check fact tables and record anomalies.
9. Write quality gate, missing fields, anomaly report, runtime trace.
10. Write handoff packet.
11. Run live/runtime verification when the user asks whether the skill can actually run:
   - Use `gmgn_okx_readonly_adapter.run_readonly_adapter_for_token(token, output_root=..., limit=8, include_kline=False, allow_network=True)` to test real data adapters and stage output mapping.
   - Use `gmgn_live_adapter.collect_and_build_source_wallet_packet(token, output_root=..., limit=8)` to test raw collection, normalization, same-source evidence, wallet intelligence packet, handoff packet, manifest, and directory guard.
   - Run `python -m modules.source_wallet_bot.runner validate-package --root /root/sikk-gmgn`, `python -m modules.source_wallet_bot.runner index-fact-store --mode <mode> --token-address <token>`, and `python modules/source_wallet_bot/validate_acceptance.py` to verify contracts/schemas/package acceptance and fact-store routing.
   - Produce an eight-group runtime report under `data/source_wallet_bot/live_test/<token_address>/eight_group_runtime_test_report.json` with one PASS/FAIL object per functional group.
12. Verify no forbidden judgment/output fields appear.

Reference runtime experiment:

```text
references/runtime_experiment_4JrarqBBkE13v6kAAgmRWKwTCBxDku6t7ZquPdY7pump.md
```
