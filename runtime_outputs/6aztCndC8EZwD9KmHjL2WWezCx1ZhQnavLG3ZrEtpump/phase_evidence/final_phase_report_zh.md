# 固定 CA P01-P09 阶段证据报告：- / 6aztCndC8EZwD9KmHjL2WWezCx1ZhQnavLG3ZrEtpump

## 总结

- P01-P09 阶段数：9/9
- 真实交易：禁止；不签名、不广播、不 swap。
- 交易门控：BLOCK / RISK_MONITOR / BLOCK_REAL_TRADE
- 风险等级：HIGH
- overall_passed：True

## 阶段状态

- P01_data_fact：DEGRADED；缺失证据：missing_snapshot_field:token_symbol, missing_snapshot_field:liquidity_usd, missing_snapshot_field:sample_wallets, P01_FUNDING_PATH_MISSING
- P02_wallet_structure：DEGRADED；缺失证据：P02_WALLET_PROFILE_SAMPLE_MISSING, 资金层跳过, funding_path_missing
...
- P01_data_fact：missing_snapshot_field:token_symbol（degraded）
- P01_data_fact：missing_snapshot_field:liquidity_usd（degraded）
- P01_data_fact：missing_snapshot_field:sample_wallets（degraded）
- P01_data_fact：资金路径缺失（degraded）
- P02_wallet_structure：P02_WALLET_PROFILE_SAMPLE_MISSING（degraded）
