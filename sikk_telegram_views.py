...
def render_main_menu(index_dir: str | Path = DEFAULT_INDEX_DIR) -> Dict[str, Any]:
    ...
    rows = [
        ...
        [_button("CA 新钱包入口", "menu:wallet_source")],
    ]
...
def _format_list(value: Any, default: str = "待补") -> str:
    if isinstance(value, list):
        clean = [_text(v, "") for v in value]
        clean = [v for v in clean if v]
        return "、".join(clean) if clean else default
...
def render_wallet_source_entry(index_dir: str | Path = DEFAULT_INDEX_DIR) -> Dict[str, Any]:
    data = _load_indexes(index_dir)
    lines = [
        "【CA 新钱包入口】",
        "说明：当用户发送 ca / CA / ca <token_address> 时，默认进入 Source Wallet Bot 新钱包数据分析体系。",
        "标准主目录：data/source_wallet_bot/<mode>/<token_address>/",
        "标准两层：wallet_data/ + structure_analysis/",
        "读取优先级：新标准入口 → token 索引 → 数据护照 → 字段字典 → 旧路径映射 → 旧目录只读补查。",
        "安全边界：只读观察，不执行真实交易、不签名、不广播。",
    ]
    rows = [
        [_button("返回主菜单", "menu:main"), _button("进入系统总览", "menu:main")],
        [_button("查看新钱包目录规则", "menu:wallet_source")],
    ]
    return _payload("\n".join(lines), rows, str(data["root"] / "system_index.json"))


def render_data_dependency_entry(index_dir: str | Path = DEFAULT_INDEX_DIR) -> Dict[str, Any]:
    data = _load_indexes(index_dir)
    contract_path = Path("docs/source_wallet_bot/directory_governance/data_dependency_contract_v1.json")
    contract = _read_json(contract_path, {})
    questions = contract.get("analysis_questions", []) if isinstance(contract, Mapping) else []
    question_names = [_text(q.get("question_name"), "") for q in questions[:12] if isinstance(q, Mapping)]
    lines = [
        "【字段依赖地图 / 数据依赖地图】",
        "分组：基础判断 / 资金判断 / 结果判断 / 风险判断 / 基础设施",
        "分析问题清单：" + _format_list(question_names),
        "说明：先看分析问题 → 判断目标 → 所需字段 → 上游接口 → 原始数据 → 标准化字段 → 结构分析用途。",
        "用途：这 12 类问题是接口接入的需求源；所有钱包结构判断先查字段依赖，再进入分类或组判定。",
        "标准文件：data_dependency_map_v1.md / data_dependency_contract_v1.json / data_dependency_field_map_v1.csv",
        "安全边界：只读查询，不执行真实交易、不签名、不广播。",
    ]
    rows = [
        [_button("返回主菜单", "menu:main"), _button("CA 新钱包入口", "menu:wallet_source")],
        [_button("问题：早期钱包识别", "q:早期钱包识别"), _button("问题：钱包角色分类", "q:钱包角色分类")],
        [_button("问题：同源关系识别", "q:同源关系识别"), _button("问题：策略门禁输出", "q:策略门禁输出")],
        [_button("查看同源钱包依赖", "dep:疑似同源钱包"), _button("查看早期资金依赖", "dep:早期结构资金是否还没出完")],
        [_button("查看新钱包狙击", "dep:疑似新钱包狙击"), _button("查看同步执行组", "dep:同步执行候选组")],
        [_button("查看临时执行钱包", "dep:疑似临时执行钱包"), _button("查看Token接收钱包", "dep:Token 接收钱包 / 被动接收钱包")],
        [_button("查看分发钱包", "dep:疑似分发接收 / 分发派发钱包"), _button("查看结果鲸鱼", "dep:结果钱包 / 高结果鲸鱼")],
        [_button("查看接盘鲸鱼", "dep:接盘鲸鱼 / 套牢鲸鱼"), _button("查看基础设施", "dep:基础设施地址")],
        [_button("查看利润回收", "dep:利润回收 / 核心资金源")],
    ]
    return _payload("\n".join(lines), rows, str(contract_path))


def render_analysis_question_detail(index_dir: str | Path, callback_data: str) -> Dict[str, Any]:
    data = _load_indexes(index_dir)
    key = _safe_code(callback_data, "q:")
    contract_path = Path("docs/source_wallet_bot/directory_governance/data_dependency_contract_v1.json")
    contract = _read_json(contract_path, {})
    target = {}
    for item in contract.get("analysis_questions", []) if isinstance(contract, Mapping) else []:
        if _text(item.get("question_name"), "") == key:
            target = dict(item)
            break
    if not target:
        lines = ["【分析问题详情】", f"问题：{key or '未知'}", "状态：待补 / 未收录", "说明：请先在 data_dependency_contract_v1.json 的 analysis_questions 中定义该问题。"]
        return _payload("\n".join(lines), [[_button("返回字段依赖", "menu:data_dependency"), _button("返回主菜单", "menu:main")]], str(data["root"] / "system_index.json"))
    lines = [
        f"【分析问题：{key}】",
        f"需要回答：{_text(target.get('question_to_answer'))}",
        f"问题分组：{_text(target.get('question_group'))}",
        f"必需字段：{_as_lines(target.get('required_fields'))}",
        f"可选字段：{_as_lines(target.get('optional_fields'))}",
        f"上游接口：{_as_lines(target.get('upstream_sources'))}",
        f"原始数据：{_as_lines(target.get('raw_data_shape'))}",
        f"标准化字段：{_as_lines(target.get('normalized_fields'))}",
        f"结构分析用途：{_text(target.get('analysis_usage'))}",
        f"缺失降级规则：{_text(target.get('degradation_rules'))}",
        f"证据等级：{_text(target.get('evidence_level'))}",
        "安全边界：分析问题只读展示，不执行真实交易、不签名、不广播。",
    ]
    rows = [[_button("返回字段依赖", "menu:data_dependency"), _button("返回主菜单", "menu:main")]]
    return _payload("\n".join(lines), rows, str(contract_path))


def render_data_dependency_detail(index_dir: str | Path, callback_data: str) -> Dict[str, Any]:
    data = _load_indexes(index_dir)
    key = _safe_code(callback_data, "dep:")
    contract_path = Path("docs/source_wallet_bot/directory_governance/data_dependency_contract_v1.json")
    contract = _read_json(contract_path, {})
    target = {}
    for item in contract.get("judgment_targets", []) if isinstance(contract, Mapping) else []:
        if _text(item.get("target_name"), "") == key:
            target = dict(item)
            break
    if not target:
        lines = [
            ...
            f"目标：{key or '未知'}",
            "状态：待补 / 未收录",
            "说明：请先在 data_dependency_contract_v1.json 中定义该判断目标。",
        ]
        return _payload("\n".join(lines), [[_button("返回字段依赖", "menu:data_dependency"), _button("返回主菜单", "menu:main")]], str(data["root"] / "system_index.json"))
    lines = [
        f"【{key}】",
        f"判断目标：{_text(target.get('judgment_target'))}",
        f"必需字段：{_as_lines(target.get('required_fields'))}",
        f"可选字段：{_as_lines(target.get('optional_fields'))}",
        f"上游接口：{_as_lines(target.get('upstream_sources'))}",
        f"原始数据：{_as_lines(target.get('raw_data_shape'))}",
        ...
        f"结构分析用途：{_text(target.get('analysis_usage'))}",
        f"缺失降级规则：{_text(target.get('degradation_rules'))}",
        "安全边界：字段依赖只读展示，不执行真实交易、不签名、不广播。",
    ]
    rows = [[_button("返回字段依赖", "menu:data_dependency"), _button("返回主菜单", "menu:main")]]
    return _payload("\n".join(lines), rows, str(contract_path))


def render_data_dependency_group(index_dir: str | Path, callback_data: str) -> Dict[str, Any]:
    data = _load_indexes(index_dir)
    group = _safe_code(callback_data, "depgroup:")
    contract_path = Path("docs/source_wallet_bot/directory_governance/data_dependency_contract_v1.json")
    contract = _read_json(contract_path, {})
    targets = []
    for item in contract.get("judgment_targets", []) if isinstance(contract, Mapping) else []:
        if _text(item.get("group"), "") == group:
            targets.append(_text(item.get("target_name"), ""))
    lines = [
        f"【字段依赖组：{group}】",
        f"组内目标：{_format_list([t for t in targets if t])}",
        "说明：点击具体目标后可查看该目标的字段、降级规则与验证路径。",
        "安全边界：字段依赖组只读展示，不执行真实交易、不签名、不广播。",
    ]
    rows = [[_button("返回字段依赖", "menu:data_dependency"), _button("返回主菜单", "menu:main")]]
    return _payload("\n".join(lines), rows, str(contract_path))
...
def render_by_callback(index_dir: str | Path, callback_data: str) -> Dict[str, Any]:
    ...
    if callback_data == "menu:wallet_source":
        return render_wallet_source_entry(index_dir)
    if callback_data == "menu:data_dependency":
        return render_data_dependency_entry(index_dir)
    if callback_data.startswith("q:"):
        return render_analysis_question_detail(index_dir, callback_data)
    if callback_data.startswith("depgroup:"):
        return render_data_dependency_group(index_dir, callback_data)
    if callback_data.startswith("dep:"):
        return render_data_dependency_detail(index_dir, callback_data)
