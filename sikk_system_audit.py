...
def _dashboard_missing(live_state_payload: Any) -> Dict[str, Any]:
    ...
    for row in tokens:
        ...
        okx_cluster_ctx = row.get("okx_cluster") if isinstance(row.get("okx_cluster"), Mapping) else {}
        flattened = dict(row)
        ...
        if isinstance(okx_cluster_ctx, Mapping):
            flattened.update({k: v for k, v in okx_cluster_ctx.items() if k not in flattened or flattened.get(k) in (None, "", [])})
        missing = _missing_fields(flattened, DASHBOARD_FIELDS + V03_REQUIRED_FIELDS + V04_OKX_CLUSTER_FIELDS)
        nested = {
...
def _recommendations(payload: Mapping[str, Any]) -> List[str]:
    ...
    if payload["dashboard_missing_fields"].get("missing_field_counts"):
        recs.append("升级 dashboard live_state 事件级字段，覆盖发现→判断→入场→持仓→退出，并接入 chip_control / market_cap_context / lifecycle v0.3 与 OKX cluster v0.4 字段。")
