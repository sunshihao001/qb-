# P06 Acceptance Check｜phase_06_strategy_gate｜策略过滤层
...
## 阶段数据检查
确认 `p06_stage_data.md` 包含目标、定位、上游/下游 handoff、输入合约、输出合约、字段、推理、反证、hard negative、状态码、missing、降级、阻断、验收、gaps。
...
## pytest 检查
运行 `python3 -m pytest tests/phase_06_strategy_gate -q`，失败则 REJECTED。
...
## 越权输出检查
禁止输出下游最终结论；本阶段禁止项：直接执行、直接写 paper_positions_open、绕过 P07。
...
## 最终状态
`P06_ACCEPTANCE_READY | P06_ACCEPTANCE_READY_WITH_GAPS | P06_ACCEPTANCE_REJECTED`。

## Wave3 counter-evidence / no-trade 验收
- 检查 P06 是否读取 P05 handoff 并继承 hard negative、missing、gaps。
- 检查每个 strategy candidate 是否有 `counter_evidence`；缺失则 `P06_ACCEPTANCE_REJECTED`。
- 检查输出中不得出现真实交易、swap、sign、broadcast、私钥/secret 请求；出现即 REJECTED。
