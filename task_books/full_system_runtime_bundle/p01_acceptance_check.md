# P01 Acceptance Check｜phase_01_data_fact｜数据事实层
...
## 边界复核
继承文件头部 `boundary`；acceptance 只做阶段任务包与 runtime 证据验收，不替 P01 生成钱包结构、同源判断、筹码控制、场景判断、策略判断或 PAPER_READY。

## 阶段数据检查
确认 `p01_stage_data.md` 包含目标、定位、上游/下游 handoff、输入合约、输出合约、字段、推理、反证、hard negative、状态码、missing、降级、阻断、验收、gaps。
...
## pytest 检查
运行 `python3 -m pytest tests/phase_01_data_fact -q`，失败则 REJECTED。
...
## 越权输出检查
禁止输出下游最终结论；本阶段禁止项：钱包结构、同源判断、筹码控制、场景判断、策略判断、PAPER_READY。
...
## 最终状态
`P01_ACCEPTANCE_READY | P01_ACCEPTANCE_READY_WITH_GAPS | P01_ACCEPTANCE_REJECTED`。
