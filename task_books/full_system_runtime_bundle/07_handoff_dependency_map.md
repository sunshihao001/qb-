...
## Handoff 链
P01 data_fact → P02 wallet_structure → P03 chip_control → P04 scenario_recognition → P05 structure_position → P06 strategy_gate → P07 execution_risk → P08 review_learning → P09 system_upgrade。

## 共享路径规则
每阶段必须写本地 handoff 与 `data/shared_handoff/<phase>/<token>/phase_xx_handoff_packet.json`（Wave 执行时）。Task0 只定义规则，不生成业务 handoff。

## 一致性规则
shared_handoff 与阶段内部 handoff 字段不一致时 BLOCK。
