# P02 Stage Data｜phase_02_wallet_structure｜结构地址层
...
## 阶段目标
结构地址层接收上游 handoff，完成本阶段事实/判断边界内的结构化输出，并将 missing、gaps、hard negative 显式传递给下游。

## 阶段定位
- phase_id: `phase_02_wallet_structure`
- phase_code: `P02`
...
- 允许输出: 钱包画像、结构地址分类、疑似同源组、疑似分发路径、疑似回流路径、GMGN 备注、wallet_structure_decision。
...
## 禁止事项
筹码控制保留、场景识别、交易位置、PAPER_READY、禁止确定庄家。
...
## implementation plan
由 `p02_code_landing.md` 执行代码骨架/runner/test/replay 任务。

## test plan
由 `p02_acceptance_check.md` 执行目录、contract、schema、pytest、replay、handoff、安全边界检查。
