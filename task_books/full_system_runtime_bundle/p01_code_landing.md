# P01 Code Landing｜phase_01_data_fact｜数据事实层
...
## 代码落地目标
在 Wave 执行时为 `phase_01_data_fact` 建立可运行、可测试、可 replay、可 handoff 的代码骨架和最小 runtime。Task0 不落业务代码。

## 允许事项
- 创建 contracts/phase_01_data_fact/ 或 contracts/p01/ 等总控认可路径。
- 创建 schemas/phase_01_data_fact/、src/phase_01_data_fact/、tests/phase_01_data_fact/、tests/fixtures/phase_01_data_fact/。
- 写 runner、validator、handoff writer、audit writer。
- 仅输出本阶段允许范围：raw 输入、字段映射、标准化、数据质量、时间有效性、missing 报告、phase_01_handoff_packet。

## 禁止事项
钱包结构、同源判断、筹码控制、场景判断、策略判断、PAPER_READY；禁止真实签名、广播、swap、自动实盘；禁止移动/删除旧数据。
...
## tests 清单
- test_phase_01_data_fact_validator.py
- test_phase_01_data_fact_runner.py
- test_phase_01_data_fact_handoff.py
- test_phase_01_data_fact_hard_negative.py
- test_phase_01_data_fact_scope_guard.py
...
## runner 命令
`python3 -m src.phase_01_data_fact.cli --mode replay --input tests/fixtures/phase_01_data_fact/ready_case.json`

## pytest 命令
`python3 -m pytest tests/phase_01_data_fact -q`

## replay 命令
`python3 -m src.phase_01_data_fact.cli --mode replay --write-handoff --write-audit`

## 阶段输出
本地 output、handoff_packet、audit_report。

## shared_handoff 输出
`/root/sikk-gmgn/data/shared_handoff/phase_01_data_fact/<token>/phase_handoff_packet.json` 或合同索引指定路径。

## audit 输出
`/root/sikk-gmgn/reports/system_audit/phase_01_data_fact_audit.md` 或 Wave 审计文件。
...
## 下一步规则
通过后交给 `p01_acceptance_check.md`；失败进入 Patch + Regression。

## 输入 / 状态码 / 阻断 / 降级 / 验收 补强
- 输入: 上游 handoff packet、合同索引、schema 索引、required fields、missing/gaps、hard-negative inheritance、legacy read-only refs。
- 状态码: `P01_CODE_READY`、`P01_CODE_READY_WITH_GAPS`、`P01_CODE_REJECTED`、`P01_CODE_BLOCKED_BY_UPSTREAM`。
