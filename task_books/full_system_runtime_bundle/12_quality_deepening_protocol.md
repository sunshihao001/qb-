...
## 文件质量底线
每个生成文件必须包含目标、边界、输入、输出、handoff、状态码、missing、阻断、降级、测试、审计、最终状态。

## 空模板判定
只有标题、泛泛原则、没有 I/O、没有 stop condition、没有 audit 的文件标记 `NOT_PROFESSIONAL_TEMPLATE` 并重写。

## 质量审计
Task0 验证时检查关键词组：输入、输出、handoff、状态码、missing、阻断、降级、验收、audit/审计。
