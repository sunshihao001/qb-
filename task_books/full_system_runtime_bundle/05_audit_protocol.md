...
- P01-P09 三类文件完整性。
- Wave 1-4 文件完整性。
- runtime/checkpoint JSON parse。
- forbidden directory 与越权话术。

## 审计输出
- full_system_runtime_bundle_audit.md
- full_system_runtime_bundle_validation.json
- missing_gap_register.md
- waves/wave_XX audit 占位文件（Wave 执行时更新）。

## 审计原则
执行者不得自证；最终状态必须由独立验证脚本写入证据。
