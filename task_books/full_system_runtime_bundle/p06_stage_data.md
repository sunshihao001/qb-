# P06 Stage Data｜phase_06_strategy_gate｜策略过滤层
...
## 阶段目标
策略过滤层接收上游 handoff，完成本阶段事实/判断边界内的结构化输出，并将 missing、gaps、hard negative 显式传递给下游。

## 阶段定位
- phase_id: `phase_06_strategy_gate`
- phase_code: `P06`
...
- 允许输出: A+P1、证据链、风险收益比、策略适配、PAPER_READY / STRATEGY_BLOCK。
...
## 禁止事项
直接执行、直接写 paper_positions_open、绕过 P07。
...
## implementation plan
由 `p06_code_landing.md` 执行代码骨架/runner/test/replay 任务。

## test plan
由 `p06_acceptance_check.md` 执行目录、contract、schema、pytest、replay、handoff、安全边界检查。
