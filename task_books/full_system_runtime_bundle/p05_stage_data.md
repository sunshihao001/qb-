# P05 Stage Data｜phase_05_structure_position｜结构位置确认层
...
## 阶段目标
结构位置确认层接收上游 handoff，完成本阶段事实/判断边界内的结构化输出，并将 missing、gaps、hard negative 显式传递给下游。

## 阶段定位
- phase_id: `phase_05_structure_position`
- phase_code: `P05`
...
- 允许输出: POC、AVWAP、Failure Test、疲劳过滤、位置延伸、Completion 判断、phase_05_handoff_packet。
...
## implementation plan
由 `p05_code_landing.md` 执行代码骨架/runner/test/replay 任务。

## test plan
由 `p05_acceptance_check.md` 执行目录、contract、schema、pytest、replay、handoff、安全边界检查。
