...
## Work Cycle
CYCLE 1 读取总控文件；CYCLE 2 扫描当前任务包状态；CYCLE 3 登记缺口；CYCLE 4 自动生成缺失文件；CYCLE 5 质量加深；CYCLE 6 阶段审计；CYCLE 7 Wave 审计；CYCLE 8 runtime_state 更新；CYCLE 9 checkpoint_state 更新；CYCLE 10 判断是否继续；CYCLE 11 输出最终状态。

## 不中停规则
不得只建目录、只建部分阶段、只建 stage_data、只写文件不审计、只审计不更新状态。
