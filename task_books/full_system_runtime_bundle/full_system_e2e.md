...
## 目标
在 Wave1-4 通过后，对 P01-P09 全链路执行 replay、handoff 一致性、status inheritance、missing propagation、hard-negative inheritance、paper-only 安全与 P09 shadow-mode upgrade 审计。

## 准入
Wave1-4 READY/READY_WITH_GAPS，blocking_issues 为空。

## 验收
- P01-P09 每阶段本地 handoff 与 shared_handoff 一致。
- 上游 hard negative 不被下游覆盖。
- missing 不被伪装。
- `PAPER_READY` 不等于 `PAPER_EXECUTED`。
- P09 回归失败时不得生成可应用升级包。

## 输出
`FULL_SYSTEM_E2E_READY | FULL_SYSTEM_E2E_READY_WITH_GAPS | FULL_SYSTEM_E2E_REJECTED` 与 `reports/system_audit/full_system_e2e_audit.md`。
