# P03 Acceptance Check｜phase_03_chip_control｜筹码控制层
...
## 边界复核
继承文件头部 `boundary`；acceptance 只做筹码控制层任务包与 runtime 证据验收，不替 P03 生成场景最终判断、AVWAP/POC 位置判断、策略放行、执行交易或 live-ready 结论。

## 阶段数据检查
确认 `p03_stage_data.md` 包含目标、定位、上游/下游 handoff、输入合约、输出合约、字段、推理、反证、hard negative、状态码、missing、降级、阻断、验收、gaps。
...
## pytest 检查
运行 `python3 -m pytest tests/phase_03_chip_control -q`，失败则 REJECTED。
...
## 越权输出检查
禁止输出下游最终结论；本阶段禁止项：场景最终判断、AVWAP/POC 位置判断、策略放行、执行交易。
...
## 最终状态
`P03_ACCEPTANCE_READY | P03_ACCEPTANCE_READY_WITH_GAPS | P03_ACCEPTANCE_REJECTED`。
