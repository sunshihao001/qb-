# P01 Stage Data｜phase_01_data_fact｜数据事实层
...
## 阶段目标
数据事实层接收上游 handoff，完成本阶段事实/判断边界内的结构化输出，并将 missing、gaps、hard negative 显式传递给下游。

## 阶段定位
- phase_id: `phase_01_data_fact`
- phase_code: `P01`
...
- 允许输出: raw 输入、字段映射、标准化、数据质量、时间有效性、missing 报告、phase_01_handoff_packet。
...
## 禁止事项
钱包结构、同源判断、筹码控制、场景判断、策略判断、PAPER_READY。
...
## implementation plan
由 `p01_code_landing.md` 执行代码骨架/runner/test/replay 任务。

## test plan
由 `p01_acceptance_check.md` 执行目录、contract、schema、pytest、replay、handoff、安全边界检查。
