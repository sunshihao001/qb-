...
## 全局边界
- HER 按总目标→阶段目标→阶段分层→层内步骤→I/O 合约→字段/状态码→正证+反证→验收审计→下阶段交接运行。
- 所有 missing 必须写为 `missing`，不得写 `0`、空字符串或 AI 推测值。
- 所有系统推导内容必须标记 `系统推导`。

## 旧数据保护
旧数据只读、copy-only、保留 fallback；禁止移动、删除、覆盖旧目录。

## 纸面交易安全
P06 可输出 `PAPER_READY`，P07 仍必须执行 paper-only 风控；禁止自动实盘、签名、广播、swap。
