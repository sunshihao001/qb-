#!/usr/bin/env python3
"""Backup sanitized @sunqbfemxbot Hermes profile data into the SIKK Git repo.
...
It copies/redacts human-readable session/log/memory data only.
"""
...
SRC = Path("/root/.hermes/profiles/sunqbfemxbot")
DEST = REPO / "钱包数据分析" / "sunqbfemxbot"
...
def main() -> int:
    ...
    # Refuse to run if there are tracked changes outside the backup folder/script.
    status = run(["git", "status", "--porcelain"], check=True).stdout.splitlines()
    allowed_prefixes = {"钱包数据分析/", "scripts/backup_sunqbfemxbot_wallet_data.py"}
    blocking = []
    for line in status:
        path = line[3:] if len(line) > 3 else line
        # Ignore untracked existing project files outside our backup scope; this repo intentionally has many.
    ...
    (DEST / "README.md").write_text(
        "# 钱包数据分析 / sunqbfemxbot 备份\n\n"
        f"- 更新时间：`{now}`\n"
        "- 来源：Hermes profile `sunqbfemxbot`\n"
    ...
    run(["git", "add", "钱包数据分析", "scripts/backup_sunqbfemxbot_wallet_data.py"])
    diff = run(["git", "diff", "--cached", "--stat"], check=True).stdout.strip()
    if not diff:
        print("backup_status=no_changes")
        return 0

    msg = f"chore: backup sunqbfemxbot wallet data {dt.datetime.now().strftime('%Y%m%d-%H%M')}"
