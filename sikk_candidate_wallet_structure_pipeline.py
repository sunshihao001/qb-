...
from modules.source_wallet_bot.gmgn_okx_readonly_adapter import collect_wallet_structure_rows_for_token
...
def _write_guard_source_manifest(
    *,
    token_dir: Path,
    token: str,
    collector_mode: str,
    ...
    manifest = build_source_manifest(
        source_id=f"{collector_mode}:{token}:{now}",
        source_type=collector_mode,
        token_address=token,
        raw_path=token_dir / "wallet_data" / "raw",
        normalized_path=token_dir / "wallet_data" / "normalized",
    ...
    _write_json(token_dir / "manifest" / "wallet_data_guard_source_manifest.json", manifest)
    return manifest


def _write_guard_scan_report(base_dir: Path) -> Dict[str, Any]:
    scan_root = base_dir.parent if base_dir.name in {"paper", "live", "shadow"} else base_dir
    report = scan_wallet_data_contamination(scan_root)
    _write_json(base_dir / "verification" / "wallet_data_guard_contamination_scan.json", report)
    return report
...
def run_candidate_wallet_structure_pipeline(
    *,
    ...
    wallet_collector: WalletCollector | None = None,
    now: str | None = None,
    source_wallet_root: str | Path | None = None,
    collector_mode: str = "gmgn_okx_readonly",
    ...
    source_wallet_base = Path(source_wallet_root) if source_wallet_root else out / "source_wallet_bot" / "paper"
    now = now or _utc_now_text()
    results: List[Dict[str, Any]] = []

    if wallet_collector is None:
        if collector_mode == "gmgn_legacy":
            wallet_collector = default_gmgn_wallet_collector
        else:
            def wallet_collector(token: str, symbol: str) -> List[Dict[str, Any]]:  # type: ignore[no-redef]
                return collect_wallet_structure_rows_for_token(
                    token,
                    symbol,
                    output_root=source_wallet_base / token,
    ...
    for row in candidates:
        ...
        token_dir = out / token / "structure_analysis"
        wallet_refresh_started_at = now
        try:
            canonical_source_wallet_dir = source_wallet_base / token
            wallet_rows = wallet_collector(token, symbol)
            _write_guard_source_manifest(
                token_dir=canonical_source_wallet_dir,
                token=token,
            ...
            results.append({
                ...
                "source_wallet_bot_token_dir": str(canonical_source_wallet_dir),
                "canonical_source_wallet_dir": str(canonical_source_wallet_dir),
    ...
    guard_report = _write_guard_scan_report(source_wallet_base)
    payload["wallet_data_guard"] = {
        "status": guard_report.get("overall_status"),
        "scan_report": str(source_wallet_base / "verification" / "wallet_data_guard_contamination_scan.json"),
        "submodule": "modules/wallet_data_guard",
    }
    for result_row in results:
        result_row["wallet_data_guard_status"] = guard_report.get("overall_status")
    payload["处理结果"] = results
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--source-wallet-root", default=None, help="canonical data/source_wallet_bot/<mode> root; token dir is created below it")
    parser.add_argument("--collector-mode", default="gmgn_okx_readonly", choices=["gmgn_okx_readonly", "gmgn_legacy"])
    parser.add_argument("--no-network", action="store_true", help="skip external readonly collectors and record degraded evidence")
    return parser.parse_args()


def main() -> None:
    args = parse_args()
    result = run_candidate_wallet_structure_pipeline(
        candidate_states_path=args.candidate_states,
        output_dir=args.output_dir,
        source_wallet_root=args.source_wallet_root,
