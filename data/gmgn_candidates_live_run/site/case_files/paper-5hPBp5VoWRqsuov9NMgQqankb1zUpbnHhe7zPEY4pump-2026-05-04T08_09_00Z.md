...
| 字段 | 数值 |
|---|---|
| Position ID | paper-5hPBp5VoWRqsuov9NMgQqankb1zUpbnHhe7zPEY4pump-2026-05-04T08:09:00Z |
| Token | TrollGoon |
| Address | 5hPBp5VoWRqsuov9NMgQqankb1zUpbnHhe7zPEY4pump |
| 状态 | CLOSED |
| 策略 | SIKK-B 控盘箱体突破回踩 |
| 信号等级 | S4_强确认信号 |
| 入场时间 | 2026-05-04T08:09:00Z |
...
系统在 待补 发现 TrollGoon。发现时市值 待补，流动性 待补，持有人 待补。纳入观察原因：进入 GMGN/SIKK 候选观察池，等待盘型、信号和钱包结构进一步确认。
...
信号在 2026-05-04 07:28:00 UTC 触发，等级为 S4_强确认信号，类型为 SIKK-B 控盘箱体突破回踩。信号价 5.9950883e-05，信号时市值 待补。失效条件：跌破关键结构位或钱包结构转弱。
...
钱包结构在 2026-05-04T08:09:00Z 给出 WALLET_BLOCK。结构分 0.0，风险分 100.0，对手盘压力 100.0，数据质量 100.0。钱包解释：对手盘压力高，接盘/套牢筹码占比偏高；缺失字段：wallet_address, role, game_side, evidence_level；筹码控制状态机：CONTROL_MIGRATING_TO_COUNTERPARTY；筹码控制状态机：COUNTERPARTY_PRESSURE_HIGH。钱包结构在这里是纸面验证门禁，不是实盘买入授权。
...
| 入场上下文 | UNKNOWN_ENTRY |
| 买入规模 | 0.2 SOL |
| 估算 token 数量 | 1270.85338426 |

纸面仓位在 2026-05-04T08:09:00Z 入场。入场模式 live，原始报价 0.00015737456615987354，模拟入场价 0.00015737456615987354，滑点 3.0%。入场时市值 待补，相对发现时市值变化 待补，上下文为 UNKNOWN_ENTRY。本次纸面规模 0.2 SOL，约 0 USD，估算 token 数量 1270.85338426。入场依据：盘型、信号、钱包结构和 quote/security 同时满足纸面验证条件。
...
持仓过程已记录 1 条 journal。最近一次 2026-05-04T08:09:00Z，价格 0.00015737456615987354，浮动收益 0%，动作 PAPER_ENTRY，原因 新纸面仓位入场后记录首条持仓日志。

| 时间 | 价格 | 市值 | 浮盈 | 钱包状态 | 动作 | 原因 |
|---|---:|---:|---:|---|---|---|
| 2026-05-04T08:09:00Z | 0.00015737456615987354 | 待补 | 0.0 | WALLET_BLOCK | PAPER_ENTRY | 新纸面仓位入场后记录首条持仓日志 |

---

## 10. 退出

纸面仓位在 2026-05-04T09:40:47Z 退出，退出价 2.2273406871449254e-06，退出市值 待补。退出触发 待补，原因码 待补，原因：命中纸面止损。
...
本次结果为 LOSS，收益 -98.58%。需要重点复查入场是否追高、钱包结构是否误判、quote/security 是否延迟，以及退出是否过慢。
...
| security_risk_level | data/gmgn_candidates_live_run/wallet_structure/5hPBp5VoWRqsuov9NMgQqankb1zUpbnHhe7zPEY4pump/wallet_structure_decision.json |
