...
| 字段 | 数值 |
|---|---|
| Position ID | paper-FESx6q1owjzTqaysjSkcaJRBKvNAcE6TEwSuaY65pump-2026-05-04T07:56:08Z |
| Token | trollhouse |
...
钱包结构在 2026-05-04T07:56:08Z 给出 WALLET_BLOCK。结构分 32.0，风险分 100.0，对手盘压力 100.0，数据质量 100.0。钱包解释：发现分发侧钱包 2 个；对手盘压力高，接盘/套牢筹码占比偏高；缺失字段：wallet_address, role, game_side, evidence_level；筹码控制状态机：CONTROL_BREAK_OR_DISTRIBUTION；筹码控制状态机：WALLET_BLOCK；筹码控制状态机：DISTRIBUTION_ACTIVE。钱包结构在这里是纸面验证门禁，不是实盘买入授权。
...
| 入场上下文 | UNKNOWN_ENTRY |
| 买入规模 | 0.183954 SOL |
| 估算 token 数量 | 1861.04445414 |

纸面仓位在 2026-05-04T07:56:08Z 入场。入场模式 live，原始报价 9.884449540751094e-05，模拟入场价 9.884449540751094e-05，滑点 3.0%。入场时市值 待补，相对发现时市值变化 待补，上下文为 UNKNOWN_ENTRY。本次纸面规模 0.183954 SOL，约 0 USD，估算 token 数量 1861.04445414。入场依据：盘型、信号、钱包结构和 quote/security 同时满足纸面验证条件。
