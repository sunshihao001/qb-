...
| 字段 | 数值 |
|---|---|
| Position ID | paper-FcqVeXBc4S3YoVjTJGUDZrLcErtXzPPSspHiuDKpump-2026-05-04T07:29:02Z |
| Token | SPEEDRUN |
| Address | FcqVeXBc4S3YoVjTJGUDZrLcErtXzPPSspHiuDKpump |
| 状态 | OPEN |
| 策略 | SIKK-B 控盘箱体突破回踩 |
| 信号等级 | S4_强确认信号 |
| 入场时间 | 2026-05-04T07:29:02Z |
| 退出时间 | 待补 |
| 纸面仓位 | 0.2 SOL |
| 入场市值 | 待补 |
| 退出市值 | 待补 |
...
系统在 待补 发现 SPEEDRUN。发现时市值 待补，流动性 待补，持有人 待补。纳入观察原因：进入 GMGN/SIKK 候选观察池，等待盘型、信号和钱包结构进一步确认。
...
信号在 2026-05-03 06:18:00 UTC 触发，等级为 S4_强确认信号，类型为 SIKK-B 控盘箱体突破回踩。信号价 3.781434e-05，信号时市值 待补。失效条件：跌破关键结构位或钱包结构转弱。
...
| wallet_structure_score | 16.0 |
| wallet_risk_score | 100.0 |
| counterparty_pressure_score | 96.0 |
| data_quality_score | 100.0 |

钱包结构在 2026-05-04T07:29:02Z 给出 WALLET_BLOCK。结构分 16.0，风险分 100.0，对手盘压力 96.0，数据质量 100.0。钱包解释：发现分发侧钱包 23 个；对手盘压力高，接盘/套牢筹码占比偏高；缺失字段：wallet_address, role, game_side, evidence_level；筹码控制状态机：CONTROL_BREAK_OR_DISTRIBUTION；筹码控制状态机：WALLET_BLOCK；筹码控制状态机：DISTRIBUTION_ACTIVE。钱包结构在这里是纸面验证门禁，不是实盘买入授权。
...
| 入场上下文 | UNKNOWN_ENTRY |
| 买入规模 | 0.2 SOL |
| 估算 token 数量 | 1122.64171965 |

纸面仓位在 2026-05-04T07:29:02Z 入场。入场模式 live，原始报价 0.00017815122714468922，模拟入场价 0.00017815122714468922，滑点 3.0%。入场时市值 待补，相对发现时市值变化 待补，上下文为 UNKNOWN_ENTRY。本次纸面规模 0.2 SOL，约 0 USD，估算 token 数量 1122.64171965。入场依据：盘型、信号、钱包结构和 quote/security 同时满足纸面验证条件。
...
持仓过程已记录 4 条 journal。最近一次 2026-05-04T12:40:33Z，价格 0.00015836959647136397，浮动收益 -11.1%，动作 EXIT_MONITOR，原因 钱包结构出现风险，但 多轮 delta 未确认；盘型冲突未确认；市场确认不足，默认 EXIT_MONITOR。

| 时间 | 价格 | 市值 | 浮盈 | 钱包状态 | 动作 | 原因 |
|---|---:|---:|---:|---|---|---|
| 2026-05-04T07:29:02Z | 0.00017815122714468922 | 待补 | 0.0 | WALLET_BLOCK | PAPER_ENTRY | 新纸面仓位入场后记录首条持仓日志 |
| 2026-05-04T11:49:29Z | 0.00016031944872602644 | 待补 | -10.0093 | WALLET_BLOCK | EXIT_MONITOR | 钱包结构出现风险，但 多轮 delta 未确认；盘型冲突未确认；市场确认不足，默认 EXIT_MONITOR |
| 2026-05-04T12:38:35Z | 0.00015836959647136397 | 待补 | -11.1038 | WALLET_BLOCK | EXIT_MONITOR | 钱包结构出现风险，但 多轮 delta 未确认；盘型冲突未确认；市场确认不足，默认 EXIT_MONITOR |
| 2026-05-04T12:40:33Z | 0.00015836959647136397 | 待补 | -11.1038 | WALLET_BLOCK | EXIT_MONITOR | 钱包结构出现风险，但 多轮 delta 未确认；盘型冲突未确认；市场确认不足，默认 EXIT_MONITOR |
...
本次结果为 LOSS，收益 -11.1%。需要重点复查入场是否追高、钱包结构是否误判、quote/security 是否延迟，以及退出是否过慢。
...
| security_risk_level | data/gmgn_candidates_live_run/wallet_structure/FcqVeXBc4S3YoVjTJGUDZrLcErtXzPPSspHiuDKpump/wallet_structure_decision.json |
