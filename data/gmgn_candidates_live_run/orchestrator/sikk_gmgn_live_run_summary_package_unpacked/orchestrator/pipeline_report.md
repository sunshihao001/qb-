...
- 运行时间：2026-05-02T05:00:00Z
- 模式：paper/readiness
- 执行边界：只做候选发现、K线吸筹识别、纸面信号、状态机，可选进入报价 + 安全扫描 + 确认层；不执行真实 swap。

## 阶段统计

- 新币筛选：{"总扫描数": 50, "进入候选池": 49, "排除数量": 1, "S3数量": 10, "S2数量": 0, "S1数量": 39, "S0数量": 1}
- K线吸筹管道：{"读取候选数": 10, "处理候选数": 10, "成功数量": 10, "失败数量": 0}
- 信号管道：{"读取候选数": 10, "成功数量": 10, "跳过数量": 0, "失败数量": 0}
- 状态机：{"DISCOVERED": 0, "WATCHING": 39, "ACCUMULATING": 0, "READY_TO_BUY": 0, "PAPER_READY": 0, "BLOCKED": 10, "FAILED": 0, "EXITED": 0, "候选数量": 49}
- 钱包结构门禁：{"处理数量": 7, "WALLET_BLOCK": 7}
- 报价安全确认层：{"读取状态数": 49, "PAPER_READY数量": 0, "成功数量": 0, "跳过数量": 49, "失败数量": 0, "READY_FOR_CONFIRMATION": 0, "PAUSE": 0, "BLOCK": 0}
