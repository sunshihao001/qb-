...
| 字段 | 数值 |
|---|---|
| Position ID | paper-ECgweD7xkMj4bm8CcM9rusxKjyQGgdosCvVmhGUupump-2026-05-04T12:40:33Z |
| Token | TROLLIEN |
| Address | ECgweD7xkMj4bm8CcM9rusxKjyQGgdosCvVmhGUupump |
| 状态 | CLOSED |
| 策略 | SIKK-B 控盘箱体突破回踩 |
| 信号等级 | S4_强确认信号 |
| 入场时间 | 2026-05-04T12:40:33Z |
| 退出时间 | 2026-05-04T12:40:33Z |
| 纸面仓位 | 0.2 SOL |
| 入场市值 | 待补 |
| 退出市值 | 待补 |
| 净收益 | 0.0 |
...
系统在 待补 发现 TROLLIEN。发现时市值 9.665e+04 USD，流动性 2.486e+04 USD，持有人 待补。纳入观察原因：进入 GMGN/SIKK 候选观察池，等待盘型、信号和钱包结构进一步确认。
...
信号在 2026-05-04 11:43:00 UTC 触发，等级为 S4_强确认信号，类型为 SIKK-B 控盘箱体突破回踩。信号价 0.00015111259，信号时市值 待补。失效条件：跌破关键结构位或钱包结构转弱。
...
钱包结构在 2026-05-04T12:40:33Z 给出 WALLET_BLOCK。结构分 0.0，风险分 100.0，对手盘压力 72.0，数据质量 100.0。钱包解释：对手盘压力高，接盘/套牢筹码占比偏高；缺失字段：wallet_address, role, game_side, evidence_level；筹码控制状态机：CONTROL_MIGRATING_TO_COUNTERPARTY；筹码控制状态机：COUNTERPARTY_PRESSURE_HIGH。钱包结构在这里是纸面验证门禁，不是实盘买入授权。
...
|---|---|
| 入场市值 | 待补 |
| 发现时市值 | 96648.0897795599 |
| 信号时市值 | 待补 |
| 从发现到入场市值变化 | % |
| 从信号到入场市值变化 | % |
| 入场上下文 | UNKNOWN_ENTRY |
| 买入规模 | 0.2 SOL |
| 估算 token 数量 | 2346.51974661 |

纸面仓位在 2026-05-04T12:40:33Z 入场。入场模式 live，原始报价 8.523260896878352e-05，模拟入场价 8.523260896878352e-05，滑点 3.0%。入场时市值 待补，相对发现时市值变化 待补，上下文为 UNKNOWN_ENTRY。本次纸面规模 0.2 SOL，约 0 USD，估算 token 数量 2346.51974661。入场依据：盘型、信号、钱包结构和 quote/security 同时满足纸面验证条件。
...
纸面仓位在 2026-05-04T12:40:33Z 退出，退出价 8.523260896878352e-05，退出市值 待补。退出触发 待补，原因码 待补，原因：命中纸面止损。
...
本次结果暂为 FLAT/未完成，需要继续观察持仓路径和退出触发。
...
| current_market_cap_usd | data/gmgn_candidates_live_run/gmgn_new_token_filter/token_candidates.json |
| current_price | paper_position_json |
| data_quality_score | paper_position_json |
| discovery_liquidity_usd | data/gmgn_candidates_live_run/gmgn_new_token_filter/token_candidates.json |
| discovery_market_cap_usd | data/gmgn_candidates_live_run/gmgn_new_token_filter/token_candidates.json |
...
| security_risk_level | data/gmgn_candidates_live_run/quote_security/candidate_quote_security_summary.json |
| signal_level | paper_position_json |
| signal_price | paper_position_json |
| signal_reason | data/gmgn_candidates_live_run/state_machine/candidate_states.json |
...
- Case 缺失字段：candidate_discovered_at、discovery_price、discovery_holder_count、signal_market_cap_usd、early_wallet_remaining_pct、early_wallet_sold_pct、same_source_sync_sell_score、wallet_support_signals、wallet_risk_signals、quote_check_time、quote_price、gmgn_price、okx_price、kline_close_price、price_deviation_pct、security_flags、entry_market_cap_usd、unrealized_pnl_pct、exit_market_cap_usd、failure_type
- 质量层缺失字段：入场市值、持仓 journal
