...
| 字段 | 数值 |
|---|---|
| Position ID | paper-9KrdYnHHrsWYpWMxyzbmmrjcXRr7ERWpB3byNeb1pump-2026-05-02T17:22:43Z |
| Token | UNIPUMP |
...
信号在 待补 触发，等级为 S4_强确认信号，类型为 SIKK-B 控盘箱体突破回踩。信号价 0.00021903091，信号时市值 待补。失效条件：跌破关键结构位或钱包结构转弱。
...
| wallet_structure_score | 28.0 |
| wallet_risk_score | 100.0 |
| counterparty_pressure_score | 100.0 |
| data_quality_score | 100 |

钱包结构在 待补 给出 WALLET_BLOCK。结构分 28.0，风险分 100.0，对手盘压力 100.0，数据质量 100。钱包解释：发现分发侧钱包 4 个；对手盘压力高，接盘/套牢筹码占比偏高。钱包结构在这里是纸面验证门禁，不是实盘买入授权。
...
纸面仓位在 2026-05-01 19:01:00 UTC 入场。入场模式 live，原始报价 0.00031130712693739325，模拟入场价 0.00031130712693739325，滑点 待补%。入场时市值 待补，相对发现时市值变化 待补，上下文为 UNKNOWN_ENTRY。本次纸面规模 0.125662 SOL，约 待补，估算 token 数量 待补。入场依据：盘型、信号、钱包结构和 quote/security 同时满足纸面验证条件。
...
纸面仓位在 2026-05-02T17:33:17Z 退出，退出价 0.00029117248875428934，退出市值 待补。退出触发 待补，原因码 STRUCTURE_WEAKENING，原因：钱包结构触发纸面强制退出。
...
本次结果为 LOSS，收益 -6.468%。需要重点复查入场是否追高、钱包结构是否误判、quote/security 是否延迟，以及退出是否过慢。
