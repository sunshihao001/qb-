...
| 字段 | 数值 |
|---|---|
| Position ID | paper-GnqRKyQfcna6XbkoJMKkvUZUBQdtMb8oy5rH2ocpump-2026-05-02T18:38:16Z |
| Token | MSTRUMP |
...
纸面仓位在 2026-05-02 18:11:00 UTC 入场。入场模式 live，原始报价 0.0002085871699257825，模拟入场价 0.0002085871699257825，滑点 待补%。入场时市值 待补，相对发现时市值变化 待补，上下文为 UNKNOWN_ENTRY。本次纸面规模 0.2 SOL，约 待补，估算 token 数量 待补。入场依据：盘型、信号、钱包结构和 quote/security 同时满足纸面验证条件。
...
纸面仓位在 2026-05-02T18:48:46Z 退出，退出价 0.00019237449461346296，退出市值 待补。退出触发 待补，原因码 STRUCTURE_WEAKENING，原因：钱包结构触发纸面强制退出。
...
本次结果为 LOSS，收益 -7.773%。需要重点复查入场是否追高、钱包结构是否误判、quote/security 是否延迟，以及退出是否过慢。
