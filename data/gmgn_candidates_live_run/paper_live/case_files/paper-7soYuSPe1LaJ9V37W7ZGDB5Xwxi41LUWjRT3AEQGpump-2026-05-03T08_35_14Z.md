...
| 字段 | 数值 |
|---|---|
| Position ID | paper-7soYuSPe1LaJ9V37W7ZGDB5Xwxi41LUWjRT3AEQGpump-2026-05-03T08:35:14Z |
| Token | WOLVERINE |
...
钱包结构在 待补 给出 WALLET_BLOCK。结构分 32.0，风险分 100.0，对手盘压力 100.0，数据质量 100。钱包解释：发现分发侧钱包 11 个；对手盘压力高，接盘/套牢筹码占比偏高；缺失字段：wallet_address, role, game_side, evidence_level；筹码控制状态机：CONTROL_BREAK_OR_DISTRIBUTION；筹码控制状态机：WALLET_BLOCK；筹码控制状态机：DISTRIBUTION_ACTIVE。钱包结构在这里是纸面验证门禁，不是实盘买入授权。
...
纸面仓位在 2026-04-28 16:24:00 UTC 入场。入场模式 live，原始报价 0.00013302719501566442，模拟入场价 0.00013302719501566442，滑点 待补%。入场时市值 待补，相对发现时市值变化 待补，上下文为 UNKNOWN_ENTRY。本次纸面规模 0.177725 SOL，约 待补，估算 token 数量 待补。入场依据：盘型、信号、钱包结构和 quote/security 同时满足纸面验证条件。
...
持仓过程已记录 1 条 journal。最近一次 2026-05-03T08:44:33Z，价格 0.00012388303511099905，浮动收益 -6.874%，动作 EXIT_MONITOR，原因 钱包结构出现风险，但 多轮 delta 未确认；盘型冲突未确认；市场确认不足，默认 EXIT_MONITOR。

| 时间 | 价格 | 市值 | 浮盈 | 钱包状态 | 动作 | 原因 |
|---|---:|---:|---:|---|---|---|
| 2026-05-03T08:44:33Z | 0.00012388303511099905 | 待补 | -6.8739 | WALLET_BLOCK | EXIT_MONITOR | 钱包结构出现风险，但 多轮 delta 未确认；盘型冲突未确认；市场确认不足，默认 EXIT_MONITOR |

---

## 10. 退出

纸面仓位在 2026-05-03T08:45:36Z 退出，退出价 0.00012388303511099905，退出市值 待补。退出触发 待补，原因码 STRUCTURE_WEAKENING，原因：钱包结构触发纸面强制退出。
...
本次结果为 LOSS，收益 -6.874%。需要重点复查入场是否追高、钱包结构是否误判、quote/security 是否延迟，以及退出是否过慢。
